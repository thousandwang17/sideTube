 ...
 * @LastEditTime: 2023-03-12 22:04:39
...
func (h *handle) helper(d amqp091.Delivery) {
	...
	if mission.MissionType != worker.AudioType {
		log.Println("Mission Type is not audioType ", mission, err)
		d.Nack(false, false)
		return
	}
	...
	fileName, err := h.videoRepo.EncodeAudio(ctx, mission)
	if err != nil {
		log.Print(err)
		if err := h.queueRepo.ReQueue(ctx, b, delay); err != nil {
	...
	log.Println(mission.MissionID, mission.VideoId, "down!")
	AckTrue(d)
}
