 ...
 * @LastEditTime: 2023-03-11 21:10:03
...
func (h *handle) helper(d amqp091.Delivery) {
	...
	fileName, err := h.videoRepo.MergeEncodeVideo(ctx, mission)
	if err != nil {
		log.Print(err)
		if err := h.queueRepo.ReQueue(ctx, b, delay); err != nil {
	...
	if alldone, err := h.lockRepo.AccomplishbMission(ctx, mission, fileName); err != nil {
		...
		// encode
		missionByte, err := json.Marshal(worker.GenerateMpdMission{
