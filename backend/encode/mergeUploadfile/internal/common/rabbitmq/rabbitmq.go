...
func (client *RabbitClient) init(conn *amqp.Connection) error {
	...
	if err = mergeVideoQueueRegister(ch); err != nil {
		return err
	}
...
//
func mergeVideoQueueRegister(ch *amqp.Channel) error {

	queue := os.Getenv("MERGE_VIDEO_QUEUE")
	if queue == "" {
		return errors.New("merge queue name is empty")
	}

	exchange := os.Getenv("MERGE_VIDEO_EXCHANGE")
	if exchange == "" {
		return errors.New("merge exchange name is empty")
	}
	...
	if err != nil {
		log.Println(err, "Failed to declare a merge exchange")
		return err
	}
	...
	if err != nil {
		log.Println(err, "Failed to Bind merge queue and exchange")
		return err
	}
...
func dectedVideoQueueRegister(ch *amqp.Channel) error {

	queue := os.Getenv("DECTED_VIDEO_QUEUE")
	if queue == "" {
		return errors.New("dectedVideo queue name is empty")
	}

	exchange := os.Getenv("DECTED_VIDEO_EXCHANGE")
	if exchange == "" {
...
func (client *RabbitClient) PublishMergeVideo(ctx context.Context, body []byte, header amqp.Table) error {
	if !client.isReady {
	...
	err = ch.PublishWithContext(
		ctx,
		os.Getenv("MERGE_VIDEO_EXCHANGE"), // exchange
		os.Getenv("MERGE_VIDEO_QUEUE"),    // routing key
		false,                             // mandatory
		false,                             // immediate
		amqp.Publishing{
	...
	log.Printf("MERGE_VIDEO_QUEUE [x] Sent %s", body)
	return nil
}

// after handling, publish another mission to encoding video queue
func (client *RabbitClient) PublishDectedVideo(ctx context.Context, body []byte) error {
	if !client.isReady {
		return errors.New("failed to push: not connected")
	}
	...
	if err = ch.Tx(); err != nil {
		return errors.New("PublishDectedVideo failed to start tx")
	}

	if err = dectedVideoQueueRegister(ch); err != nil {
		return errors.New("rabbiqmq channel Failed to Declare mergeVideo Queue")
	}

	err = ch.PublishWithContext(
		ctx,
		os.Getenv("DECTED_VIDEO_EXCHANGE"), // exchange
		os.Getenv("DECTED_VIDEO_QUEUE"),    // routing key
		false,                              // mandatory
		false,                              // immediate
		amqp.Publishing{
			ContentType: "application/json",
	...
	log.Printf("DECTED_VIDEO_QUEUE [x] Sent %s", body)
	return nil
}
