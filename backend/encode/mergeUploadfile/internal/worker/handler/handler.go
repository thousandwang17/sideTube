...
func (h *handle) helper(d amqp091.Delivery) {
	...
	// lock this message by videoID && workerID
	...
	// merge chunks of file, if chunks  alerdy been handled, err will return nil
	if err = h.videoRepo.CompleteMultipartUpload(ctx, mission.VideoId); err != nil {
		log.Print(err)
		if err := h.queueRepo.ReQueue(ctx, b, delay); err != nil {
	...
	dectedVideoByte, err := json.Marshal(worker.DecteddMission{
	...
	if err = h.queueRepo.PublishDectedVideo(ctx, dectedVideoByte); err != nil {
		log.Print(err)
		if err := h.queueRepo.ReQueue(ctx, b, delay); err != nil {
			// if err, let message retry
			d.Nack(false, false)
